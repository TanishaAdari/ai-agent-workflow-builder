...
export default function Home(){
  ...
  async function refresh(){
    ...
    try{
      ...
      const nextOrgs=orgData.org_members||[];
      const nextWorkflows=workflowData.workflows||[];

      setOrgs(nextOrgs);
      setWorkflows(nextWorkflows);

      setSelectedOrg((current: string)=>{
        return current||nextOrgs[0]?.org_id||'';
      });

      setSelectedWorkflow((current: any)=>{
        if(!current)return null;
        return nextWorkflows.find((item:any)=>item.id===current.id)||null;
      });
    }catch(error:any){
  ...
  useEffect(()=>{
    ...
    const dispose=ws.subscribe(
      {
        query:`subscription StepRuns($runId:uuid!){step_runs(where:{workflow_run_id:{_eq:$runId}},order_by:{created_at:asc}){id status input output error attempt_count approved_by approved_at workflow_step{position type}}}`,
        variables:{
          runId:selectedWorkflow.workflow_runs?.[0]?.id
        }
      },
      {
        next:data=>setStepRuns((data as any).data.step_runs),
        error:()=>{},
        complete:()=>{}
      }
    );
      ...
      const data=await request<any>(
        ...
        CREATE_WORKFLOW,
        {
          workflow:{
            org_id:selectedOrg,
            name:newName,
            description:'LLM → HTTP → branch → approval'
          }
        }
      );
      ...
      const steps=[
        {
          ...
          type:'llm_call',
          config:{
            prompt:'Answer with exactly one word: APPROVE or REJECT. Say APPROVE.',
        ...
        },
        {
          ...
          type:'approval_gate',
          config:{
            message:'Owner or editor approval required.'
          }
        }
      ];

      for(const step of steps){
        await request(nhost,ADD_STEP,{step});
      }
  ...
  async function moveStep(step:any,direction:number){
    ...
    try{
      ...
      await request(
        ...
        {
          id:step.id,
          position:steps[target].position
        }
      );
    ...
    return (
      <main className="auth">
        <section className="card auth-card">
          <div className="brand">AgentFlow</div>
          <h1>Build AI workflows.</h1>
          <p>Sign in to your Nhost account to manage organization workflows.</p>
  ...
  return (
    <main className="shell">
      <header>
        <div>
          <div className="brand">AgentFlow</div>
          <span className="muted">AI Agent Workflow Builder</span>
        </div>
      ...
      <section className="toolbar card">
        ...
        <div className="quota">
          <span>Usage</span>
          <strong>
            {currentOrg?.organization.calls_used||0} / {currentOrg?.organization.calls_allowed||0}
          </strong>
        </div>
      </section>
      ...
      <div className="grid">
        <section className="card">
          <div className="section-title">
            <div>
              <h2>Workflows</h2>
              <p>Only workflows in your selected organization are returned.</p>
            </div>
          </div>

          {visibleWorkflows.map(workflow=>(
            <button
              className={`workflow ${selectedWorkflow?.id===workflow.id?'selected':''}`}
              key={workflow.id}
              onClick={()=>setSelectedWorkflow(workflow)}
            >
              <span>{workflow.name}</span>
              <small>
                {workflow.workflow_steps.length} steps · {workflow.workflow_runs?.[0]?.status||'never run'}
              </small>
            </button>
          ))}
        ...
        <section className="card">
          ...
          <p>Create a demo or add individual steps to the selected workflow.</p>
          ...
          <button
            disabled={
              busy||
              !currentOrg||
              (currentOrg.role!=='owner'&&currentOrg.role!=='editor')
            }
            ...
            <>
              ...
              <div className="builder-row">
                <select
                  defaultValue=""
                  onChange={e=>{
                    if(e.target.value)addTrigger(e.target.value);
                  }}
                >
                  <option value="">Attach trigger</option>
          ...
          <p className="muted">
            Sensitive steps such as db_write, notify and webhook are owner-only.
            Hasura enforces that rule too.
          </p>
        </section>
      </div>
      ...
      {selectedWorkflow&&(
        <section className="card run-panel">
          <div className="run-head">
            ...
            {canRun&&(
              <button disabled={busy} onClick={runWorkflow}>
                Run workflow
              </button>
            )}
          </div>
              ...
              <div className="step" key={step.id}>
                ...
                <div className="step-actions">
                  ...
                  <button
                    className="secondary"
                    disabled={
                      index===selectedWorkflow.workflow_steps.length-1||busy
                    }
                    onClick={()=>moveStep(step,1)}
                  >
                    ↓
          ...
          {stepRuns.length>0&&(
            <div className="live">
              ...
              {stepRuns.map(run=>(
                <div className="live-row" key={run.id}>
                  <div>
                    <strong>
                      Step {run.workflow_step.position}: {run.workflow_step.type}
                    </strong>
                  ...
                  {run.status==='paused'&&canRun&&(
                    <button onClick={()=>approve(run.id)}>
                      Approve
                    </button>
                  )}

                  <small>
                    {run.error||JSON.stringify(run.output||'')}
                  </small>
                </div>
              ))}
            </div>
